...
//, [timerRunning]
  ...
  return (
    <div className="bg-slate-500 flex flex-col justify-center items-center mt-7">
      ...
      {!isQuizStarted ? (
        <div className='start-screen bg-slate-500'>
          
          <h1 className='font-bold'>Introduction to General Anatomy</h1>
        ...
        <div className='quiz-container bg-slate-500'>
          ...
          <div className='bg-slate-500 flex flex-col justify-start items-center'>
          <h1 className='font-bold text-black text-xl'>Introduction to General Anatomy</h1>
          ...
          <h3 className='font-bold bg-slate-500'>{question}</h3>
          <ul>
            {answers.map((answer: string, idx: number) => (
              <li
                key={idx}
                onClick={() => onAnswerSelected(answer, idx)}
                className={selectedAnswerIndex === idx ? 'li-selected' : 'li-hover font-bold bg-slate-500'}              
              >
                <span>{answer}</span>
              </li>
            ))}
